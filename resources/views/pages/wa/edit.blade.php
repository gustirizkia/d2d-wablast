            ...
            <form action="{{ route("admin.wa.store") }}" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-12 mb-3">
                        ...
                        <small class="text-secondary">*Kosongkan jika tidak ingin ganti image</small>
                    </div>
                    <div class="col-md-12 mb-3">
                        <label for="" class="form-label">Message</label>
                        <textarea name="message" id="" cols="30" rows="4" class="form-control" required>{{ $message->message }}</textarea>
                    </div>
                </div>
                ...
                <div class="row">
                    @foreach ($soals as $soal)
                        <div class="col-12 mb-4">
                            ...
                            @php
                                $jawaban = null;
                                $is_soal = $messageHasJawaban->contains(function($val, $key)use($soal){
                                            if($val->soal_id === $soal->id){
                                                return true;
                                            }else{
                                                return false;
                                            }
                            ...
                            @else
                                @foreach ($soal->pilihan as $pilihan)
                                    <div class="form-check">
                                        <input class="form-check-input" name="jawaban_id[]" type="checkbox" value="{{ $pilihan->id }}" {{ $messageHasJawaban->contains("pilihan_id", $pilihan->id) ? "checked" : "" }} id="flexCheckDefault{{ $soal->id }}_{{ $pilihan->id }}">
